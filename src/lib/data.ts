export const BUSINESS = {
  name: "The Patch Boys of Bergen County",
  phone: "(201) 630-7181",
  email: "almatesic@gmail.com",
  tagline: "Seamless Patches. Superior Service.™",
  description: "Bergen County's trusted drywall repair experts. Fast, professional drywall and ceiling repairs with free estimates and same-day service.",
  address: "Ridgewood, NJ 07450",
  addressLocality: "Ridgewood",
  addressRegion: "NJ",
  postalCode: "07450",
  hours: "Mon-Fri: 8AM-6PM | Sat: 9AM-3PM",
  googleMapsEmbed: "",
};

export type ServiceProcessStep = { title: string; description: string };
export type ServiceIssue = { problem: string; solution: string };
export type ServiceFAQ = { q: string; a: string };

export type Service = {
  slug: string;
  title: string;
  shortDesc: string;
  longDesc: string;
  icon: string;
  benefits: string[];
  keywords: string[];
  introParagraphs: string[];
  processSteps: ServiceProcessStep[];
  commonIssues: ServiceIssue[];
  pricingInfo: {
    range: string;
    factors: string[];
    note: string;
  };
  serviceFAQs: ServiceFAQ[];
};

export const SERVICES: Service[] = [
  {
    slug: "drywall-repair",
    title: "Drywall Repair",
    shortDesc: "Expert patching and repair for holes, cracks, dents, and water damage in your drywall.",
    longDesc: "Whether it's a small nail hole or a large gash from moving furniture, our skilled technicians restore your walls to their original condition. We handle all types of drywall damage including holes, cracks, water damage, and settling cracks with precision and care.",
    icon: "🔧",
    benefits: ["Same-day repairs available", "Invisible patch results", "All hole sizes handled", "Water damage specialists"],
    keywords: ["drywall repair", "drywall patching", "hole repair", "wall repair", "sheetrock repair"],
    introParagraphs: [
      "Damaged drywall is one of the most common repair needs in Bergen County homes, and it rarely happens at a convenient time. A doorknob punched through the wall, a bed frame dragged across the sheetrock, a leak behind the washing machine — these everyday accidents leave holes, dents, and cracks that make an otherwise well-kept home look neglected. The Patch Boys of Bergen County specializes in drywall repair that restores walls to a truly invisible finish, so you can stop staring at that hole every time you walk through the room.",
      "We repair everything from pinholes and hairline cracks to large gashes, door-handle punches, drywall anchor craters, and sections damaged by water or impact. Our technicians carry all the tools and materials needed to complete most repairs in a single visit, including 1/2-inch and 5/8-inch drywall, joint compound, mesh and paper tape, corner bead, texture sprayers, and color-matched primer. We arrive in clean, marked vehicles, protect your floors and furniture with drop cloths and plastic, and leave the workspace cleaner than we found it.",
      "What sets our drywall repair apart is our obsession with invisible results. Anyone can slap mud on a hole. Matching the surrounding texture, feathering the compound edges, and blending the paint so the patch disappears takes craft — and it's what we do every day. Homeowners across Hackensack, Paramus, Teaneck, Ridgewood, Fair Lawn, and the rest of Bergen County call us because they've been burned by handymen who left visible patches. We fix those too.",
    ],
    processSteps: [
      { title: "Free On-Site Estimate", description: "We inspect the damage, identify the root cause (impact, settling, water), and give you a fixed, written price before any work begins. No surprises." },
      { title: "Surface Prep & Containment", description: "We protect floors and nearby furniture with drop cloths, tape off the work area, and cut back the damaged drywall to solid edges." },
      { title: "Patch Installation", description: "For small holes we use a California patch or hot patch; for larger holes we cut a clean rectangle, install backing, and secure a new drywall piece flush to the existing surface." },
      { title: "Taping & Mudding", description: "We apply joint tape and three coats of joint compound, feathering each coat wider than the last and sanding between coats for a seamless transition." },
      { title: "Texture Matching", description: "We spray or roll texture to match your existing orange peel, knockdown, smooth, or hand-troweled finish so the patch blends perfectly." },
      { title: "Prime & Paint Touch-Up", description: "We prime the repair and touch up the paint using your existing color or a close match, leaving a wall that looks like the damage never happened." },
    ],
    commonIssues: [
      { problem: "Doorknob holes and furniture impact damage", solution: "We cut back to solid drywall, install a backer board, patch with new drywall, and finish flush with the surrounding wall." },
      { problem: "Nail pops and screw pops from settling", solution: "We remove or reset the fastener, drive a new screw above and below, and finish with three coats of compound." },
      { problem: "Drywall anchor craters from removed shelving or TVs", solution: "We remove the anchor, fill with setting-type compound, apply tape if needed, and finish with texture matching." },
      { problem: "Cracks at corners, seams, and above doorways", solution: "We re-tape the seam with paper or mesh tape and apply three coats of compound to prevent the crack from returning." },
      { problem: "Water-damaged or stained drywall", solution: "We cut out the damaged section, verify the leak is fixed, install new drywall, and finish with stain-blocking primer and matched paint." },
      { problem: "Sagging or bulging drywall from moisture", solution: "We remove the compromised section back to studs, check framing and insulation, and install new moisture-resistant drywall where appropriate." },
    ],
    pricingInfo: {
      range: "Most drywall repairs in Bergen County run $150–$500",
      factors: ["Size and number of holes or damaged areas", "Texture complexity (smooth vs. knockdown vs. hand-troweled)", "Whether painting and priming is included", "Accessibility of the work area", "Ceiling repairs vs. wall repairs"],
      note: "Every job starts with a free, fixed-price estimate. You'll know the full cost in writing before we start.",
    },
    serviceFAQs: [
      { q: "How long does a drywall repair take?", a: "Most small-to-medium drywall repairs are completed in a single visit lasting 2–4 hours. Larger repairs or jobs that require compound to dry fully between coats may need a second visit the following day." },
      { q: "Will the patch be noticeable?", a: "When done correctly, no. Our texture matching and paint blending process is designed to make the repair invisible under normal lighting. We don't leave until you're satisfied the patch has disappeared." },
      { q: "Do you handle large holes and whole-panel replacement?", a: "Yes. Whether it's a 2-inch doorknob hole or a 4-foot section damaged by a plumbing leak, we have the materials and experience to handle it in a single service call." },
      { q: "Can you repair drywall damaged by water?", a: "Yes, as long as the water source has been stopped. We remove the damaged drywall, inspect the framing, install new sheetrock, and finish with stain-blocking primer and color-matched paint." },
      { q: "Is there a difference between sheetrock repair and drywall repair?", a: "No — they're the same thing. \"Sheetrock\" is a USG Corporation brand name for gypsum drywall that became genericized, like Kleenex for tissue or Xerox for photocopies. When Bergen County homeowners call asking for sheetrock repair, we're doing exactly what we'd do for drywall repair: cutting out the damaged panel, installing new gypsum board, taping, mudding, texture matching, and painting. Same materials, same process, same price — different word." },
    ],
  },
  {
    slug: "emergency-drywall-repair",
    title: "Emergency Drywall Repair",
    shortDesc: "Same-day drywall and ceiling repair for water damage, ceiling collapses, listing-day emergencies, and insurance claims across Bergen County, NJ.",
    longDesc: "When drywall damage can't wait — a ceiling bulging from a leak, a hole in the wall before tomorrow's showing, a tenant-caused crater before a move-in — we dispatch same-day. Our emergency drywall repair service covers water damage, impact damage, ceiling emergencies, and insurance claim work across Bergen County with a transparent, no-surcharge pricing structure.",
    icon: "🚨",
    benefits: ["Same-day dispatch across Bergen County", "No emergency or weekend surcharges", "Water-damage and mold-aware repairs", "Insurance documentation included", "Fully insured and licensed crew", "Clean, covered, and respectful on site"],
    keywords: ["emergency drywall repair", "same-day drywall repair", "24 hour drywall repair", "urgent drywall repair Bergen County", "insurance drywall repair NJ", "ceiling emergency repair"],
    introParagraphs: [
      "Some drywall jobs can wait a week. Others can't wait an afternoon. A ceiling bulging from an upstairs leak, a fist-sized hole discovered the night before a home showing, a tenant who left a crater in the hallway and the new lease starts Monday — these are the calls we built our same-day service for. Emergency drywall repair in Bergen County means you pick up the phone, we pick up the truck, and the damage that's costing you money or peace of mind is handled before the day ends on most jobs.",
      "We treat \"emergency\" literally, not as a pricing gimmick. There's no 3x after-hours rate, no weekend surcharge, no minimum call-out fee designed to punish you for calling. You pay the same transparent, fixed-price estimate whether you called us on a Tuesday morning or a Saturday afternoon — we just move you to the front of the queue when the damage is urgent. Bergen County homeowners and property managers call us for water-damaged ceilings after a pipe burst, for impact damage the day before a listing goes live, for vandalism or tenant-turnover repairs, for insurance adjuster deadlines, and for mid-renovation surprises their contractor didn't flag.",
      "What makes same-day emergency drywall repair actually work is preparation. Our trucks carry a full complement of 1/2-inch and 5/8-inch drywall, moisture-resistant green board, setting-type and all-purpose compounds, mesh and paper tape, corner bead, primer, texture sprayers, and portable drying equipment. That means we can arrive, assess, stabilize, and finish most emergency repairs in a single visit — not a site inspection today, materials order tomorrow, return visit next week. For water damage where drying time is non-negotiable, we break the job into two visits (demo and dry today, finish in 48 hours) and price it accordingly up front. Either way, you know exactly what you're paying and when you're getting your wall back before we lift a hammer.",
    ],
    processSteps: [
      { title: "Urgent Intake Call", description: "You call or text our main line. We ask three questions — what happened, when, and where in Bergen County — and give you a realistic arrival window, usually within 2–6 hours for true emergencies." },
      { title: "Rapid Dispatch", description: "A fully stocked truck and technician are routed to your address. You get a text when we're en route with an accurate ETA so you're not stuck waiting all day." },
      { title: "On-Site Assessment & Fixed Quote", description: "We assess the damage, identify any active sources (leaks, moisture behind drywall, structural concerns), and give you a fixed, written price before any work starts. You can still say no with zero obligation." },
      { title: "Stabilize First", description: "If there's active water, sagging ceiling material, or safety risk, we stabilize the scene immediately — remove compromised drywall, set up containment, and start any drying needed — before any finish work begins." },
      { title: "Same-Visit Repair When Possible", description: "For dry impact damage, listing-prep holes, and most non-water jobs, we complete the cut, patch, tape, mud, texture, prime, and paint touch-up in a single visit. You go from \"my ceiling has a hole\" to \"the hole never happened\" in 2–5 hours." },
      { title: "Documentation for Insurance", description: "If your job is an insurance claim, we provide before/during/after photos, a written scope of work, itemized invoice, and the damage cause description adjusters need. We've worked with every major carrier operating in New Jersey." },
    ],
    commonIssues: [
      { problem: "Ceiling water damage from an upstairs leak, bathroom, or roof", solution: "We locate the source (or coordinate with your plumber), remove the saturated drywall, dry the cavity, verify the leak is stopped, and reinstall moisture-resistant drywall with a stain-blocking primer and color-matched paint." },
      { problem: "Sagging or bulging ceiling about to fall", solution: "We treat this as a same-day priority. We stabilize the area, remove the compromised ceiling section safely, check the joists and insulation, and install new drywall with proper taping and finish." },
      { problem: "Last-minute hole repair before a listing, open house, or appraisal", solution: "We arrive with drywall, compound, texture, and paint matched to your surroundings, and finish the repair invisibly in a single visit so your Zillow photos and in-person showings look pristine." },
      { problem: "Tenant-caused damage during a rental turnover", solution: "We repair all damage — holes, anchor craters, impact dents, water stains — and provide an itemized invoice suitable for deducting against the security deposit, including photos and a written scope of work." },
      { problem: "Insurance claim drywall repair after water, fire, or storm damage", solution: "We coordinate directly with your adjuster, provide the documentation packet they need, and complete the repair once the claim is approved. We can also provide a scope and estimate before you file, so you know what you're claiming." },
      { problem: "Mid-renovation surprises (unexpected wall damage, plumbing rework, discovered mold)", solution: "We respond same-day to keep your renovation on schedule. We coordinate with your GC or plumber, repair the drywall properly, and hand the space back ready for paint or tile." },
    ],
    pricingInfo: {
      range: "Most emergency drywall repairs in Bergen County run $250–$800",
      factors: ["Extent of damage (single hole vs. full ceiling section vs. multi-room)", "Whether water damage requires drying time and a second visit", "Size of the damaged area and texture complexity", "Whether primer and paint touch-up is included", "Insurance documentation and photo packet (no upcharge — included at no cost)"],
      note: "Same transparent pricing as our standard service — no emergency surcharge, no weekend premium, no minimum call-out fee. You get a fixed written quote before any work begins.",
    },
    serviceFAQs: [
      { q: "How fast can you actually get here?", a: "For true emergencies in Bergen County — active water damage, ceiling about to collapse, listing-day holes — we typically arrive within 2–6 hours of your call. For non-emergency same-day work, we'll give you a realistic window when you call. We don't over-promise arrival times we can't hit." },
      { q: "Do you charge extra for same-day or weekend work?", a: "No. Our pricing is the same whether you call Tuesday morning or Saturday afternoon. We think \"emergency pricing\" is how the home services industry gets a bad name — we'd rather earn repeat business with transparent, fixed-price estimates." },
      { q: "Do you work with insurance companies?", a: "Yes. We provide the documentation packet adjusters need (photos, written scope, itemized invoice, damage-cause description) and have worked with every major carrier operating in New Jersey. We can also provide a pre-claim estimate if you're deciding whether to file." },
      { q: "What if the damage involves water — can you finish in one day?", a: "If the cavity behind the drywall needs to dry (most water-damage jobs), we break the work into a same-day demo-and-dry visit plus a finish visit 24–48 hours later. We explain this up front and quote both visits in the original estimate — no surprise add-ons." },
      { q: "What counts as an \"emergency\" for you?", a: "Anything where waiting costs you money, creates safety risk, or blocks something time-sensitive. Typical examples: active or recent water damage, ceiling showing signs of imminent failure, damage before a home showing or closing, tenant turnover on a deadline, mid-renovation surprises, and insurance-adjuster timelines. If you're not sure, call — we'll tell you honestly whether it needs a same-day dispatch or can be scheduled normally." },
      { q: "Are you licensed and insured for emergency work?", a: "Yes. We carry full general liability and workers' comp insurance, operate under New Jersey home improvement contractor registration, and can provide COI (certificates of insurance) to property managers, HOAs, and commercial clients that require them before work begins." },
    ],
  },
  {
    slug: "insurance-claim-drywall-repair",
    title: "Insurance Claim Drywall Repair",
    shortDesc: "Drywall and ceiling restoration for water, fire, storm, and impact damage claims — full adjuster documentation and direct-bill coordination across Bergen County.",
    longDesc: "Insurance claim work has its own rhythm — photos, scope, adjuster approval, supplements, Xactimate line items. We handle the documentation and coordination so your claim moves cleanly, the repair matches your pre-loss condition, and you're not out of pocket more than your deductible. We specialize in drywall and ceiling restoration after water damage, fire and smoke damage, storm and tree impact, vandalism, and sudden accidental discharge events.",
    icon: "📋",
    benefits: ["Adjuster-ready photo and scope documentation", "Xactimate-compatible line items on every estimate", "Direct communication with your carrier", "Supplement requests handled for hidden damage", "Pre-loss condition matching (texture, paint, trim)", "No out-of-pocket beyond your deductible on most claims"],
    keywords: ["insurance claim drywall repair NJ", "water damage drywall repair Bergen County", "fire damage drywall restoration", "storm damage wall repair NJ", "insurance drywall contractor", "xactimate drywall contractor"],
    introParagraphs: [
      "Filing a homeowner's insurance claim for drywall damage is stressful enough without also finding a contractor who understands how insurance actually works. Water damage from a burst pipe, a ceiling that came down after an ice dam, smoke and soot throughout a hallway after a kitchen fire, a tree limb through the guest room wall after a nor'easter — these are the claims Bergen County homeowners call us for, and the reason they call us specifically is because we speak the insurance language that keeps claims moving instead of bouncing between your adjuster and three contractors who don't reply to emails.",
      "Our insurance claim drywall repair process is built around the documentation adjusters need to approve and pay. Every estimate we provide for a claim is itemized with the labor, materials, and scope language that matches how insurance carriers scope restoration work (including Xactimate-compatible pricing when your adjuster uses it). Every job starts with a thorough photo set of pre-existing conditions and damage, and finishes with a matching photo set showing the completed repair. When we discover hidden damage behind the drywall — additional moisture, compromised framing, mold, insulation that needs replacement — we document it and submit a supplement request to your adjuster on your behalf rather than letting it derail your claim.",
      "We've worked with every major carrier operating in New Jersey — State Farm, Allstate, Travelers, Liberty Mutual, Chubb, Geico, USAA, Nationwide, Plymouth Rock, NJM, and most independent specialty carriers. We understand the difference between actual cash value and replacement cost value, how depreciation recovery works, and what it takes to get your repair covered at pre-loss condition rather than a patch-and-paint that shows. The goal on every claim job is simple: you pay your deductible, your adjuster pays the approved scope, and your walls and ceilings look like the damage never happened.",
    ],
    processSteps: [
      { title: "Free Claim Consultation", description: "Call us before or after you file. We'll walk the damage with you, explain what the repair will involve, and tell you honestly whether filing is worth it given your deductible and the scope of damage." },
      { title: "Documentation Photos & Scope", description: "We photograph everything — damaged areas, room context, affected finishes, and any moisture or structural concerns — and write a detailed scope of work with itemized line items in the format your carrier expects." },
      { title: "Adjuster Coordination", description: "We communicate directly with your adjuster on your behalf, meet them on site if needed for the scope walk, and reconcile any differences between their estimate and ours before work starts so there are no payment surprises." },
      { title: "Stabilize & Mitigate", description: "If there's active water, compromised ceiling material, or any other condition that could cause further damage, we stabilize immediately to prevent your claim from growing (most policies require reasonable mitigation to maintain coverage)." },
      { title: "Restoration to Pre-Loss Condition", description: "We demo damaged drywall, dry the cavity if needed, install new drywall matching original thickness and type (including moisture-resistant board where appropriate), and finish with texture matching, priming, and color-matched paint so the repair is invisible." },
      { title: "Supplement Requests & Final Documentation", description: "If we uncover hidden damage behind walls during demo, we document it, pause, and submit a supplement request to your adjuster before proceeding — so the additional work is covered, not an out-of-pocket surprise. At project close, we deliver a complete before/during/after photo packet, final invoice, and scope sign-off for your claim file." },
    ],
    commonIssues: [
      { problem: "Water damage from burst pipes, washing machine hoses, or dishwasher supply lines", solution: "We remove saturated drywall and insulation, verify drying with moisture meters, install new drywall (moisture-resistant where appropriate), and finish with stain-blocking primer and color-matched paint. We document moisture readings for your claim file." },
      { problem: "Ceiling damage from roof leaks, ice dams, or upstairs plumbing", solution: "We confirm the source is resolved, remove compromised ceiling drywall back to joists, dry the cavity, check for mold, and reinstall with proper taping, texture matching, and paint. Photos at every stage for your adjuster." },
      { problem: "Fire and smoke damage to walls and ceilings", solution: "We handle the drywall portion of fire restoration — removing smoke-saturated drywall and insulation where odor is trapped, sealing framing with odor-blocking primer, and installing new drywall matched to original finish. Coordinates with any remediation contractor on the claim." },
      { problem: "Storm damage — tree impact, wind-driven rain, roof penetration", solution: "We repair impact damage to walls and ceilings, address secondary water damage from the opening, and restore finishes to pre-loss condition. Common on storm claims after Bergen County nor'easters and hurricanes." },
      { problem: "Vandalism, break-in, or tenant-caused damage on a filed claim", solution: "We repair impact damage, hole damage, and anchor craters, and provide the detailed itemized documentation that police-report-backed vandalism and malicious-damage claims require." },
      { problem: "Hidden damage discovered mid-repair (moisture, mold, framing, insulation)", solution: "We stop, document the discovery with photos and written description, submit a supplement request to your adjuster, and resume once approved — so the additional scope is covered rather than becoming an out-of-pocket expense." },
    ],
    pricingInfo: {
      range: "Insurance claim drywall repairs in Bergen County typically run $500–$5,000+ depending on scope",
      factors: ["Square footage of damaged drywall and ceiling", "Whether framing, insulation, or moisture remediation is also required", "Texture complexity and paint matching requirements", "Single-room vs. multi-room scope", "Supplements for hidden damage discovered during demo"],
      note: "Most claim jobs settle at zero out-of-pocket beyond your deductible. We bill the carrier directly for the approved scope and handle supplements for any hidden damage uncovered during repair.",
    },
    serviceFAQs: [
      { q: "Should I file a claim for my drywall damage?", a: "We're happy to give you an honest read before you file. Call us, describe the damage, and we'll give you a ballpark repair cost. If the repair is close to or below your deductible, filing may not be worthwhile. If it's substantially above, filing usually makes sense. We'd rather give you the honest answer than push you into a claim that won't pay out." },
      { q: "Do you work directly with my insurance company?", a: "Yes. We've worked with every major carrier operating in New Jersey and can communicate directly with your adjuster, provide Xactimate-compatible estimates, meet on site for the scope walk, and submit supplements as needed. You don't have to be in the middle of every email." },
      { q: "What if the adjuster's estimate is lower than yours?", a: "This is common and almost always resolvable. We reconcile differences line-by-line with the adjuster before work begins — usually by explaining scope items they missed or by providing photos that support the line items we included. If you'd rather a public adjuster handle it, we can recommend one." },
      { q: "What happens if you find more damage behind the drywall during demo?", a: "We stop, photograph the discovery, write up a supplement request with the additional scope and cost, and submit it to your adjuster for approval before we proceed. This is standard claim process — it protects you from out-of-pocket surprises." },
      { q: "Will my repaired walls look like they did before the damage?", a: "Yes — pre-loss condition matching is a standard carrier expectation and what we aim for on every claim job. We match texture (smooth, orange peel, knockdown, hand-troweled), match paint color, and feather patches so the repair is invisible under normal lighting." },
      { q: "Do I pay you directly or does the insurance company pay you?", a: "In most cases we bill your carrier directly for the approved scope and you're only responsible for your deductible. If your carrier pays you directly (common with some policies), you endorse the check to us for the approved portion. We'll walk you through whichever path your carrier uses." },
    ],
  },
  {
    slug: "property-manager-drywall-services",
    title: "Property Manager Drywall Services",
    shortDesc: "Fast, documented drywall and ceiling repair for landlords, property managers, and realtors across Bergen County — tenant turnovers, listing prep, and portfolio maintenance.",
    longDesc: "Property managers and landlords don't need another handyman — they need a drywall contractor who understands turnover timelines, documentation for security-deposit disputes, consistent quality across multi-unit portfolios, and scheduling that moves faster than a month-long waitlist. We serve landlords, property management companies, realtors, and small multi-family owners throughout Bergen County.",
    icon: "🏢",
    benefits: ["Same-week scheduling for tenant turnovers", "Itemized invoices for security-deposit deductions", "Portfolio pricing for 5+ unit landlords", "Consistent crew and quality across properties", "Certificate of insurance on file", "Before/after photo documentation standard"],
    keywords: ["drywall repair for landlords NJ", "property management drywall contractor", "tenant turnover drywall repair Bergen County", "multi-unit drywall repair NJ", "realtor drywall repair", "commercial drywall repair Bergen County"],
    introParagraphs: [
      "Running a rental portfolio in Bergen County means the drywall never stops needing attention. Tenant moves out, punches a hole, gouges the hallway with a couch, leaves a dozen anchor craters from wall art — and you've got a week to turn the unit before the new lease starts. Or a showing tomorrow on a listing you're representing, and the seller called about a hole you didn't know existed. These are the calls property managers, landlords, and realtors make to us specifically, because we understand that your deadline is the actual business constraint, not a rough preference.",
      "Our property manager drywall service is built around three things most contractors get wrong for this buyer: scheduling, documentation, and consistency. On scheduling, we hold turnover slots open for our portfolio clients so you're not competing with homeowners for same-week availability. On documentation, every job ships with itemized line items (labor, materials, specific rooms affected) and before/during/after photos — the packet you need if you're deducting against a security deposit, reporting to an HOA, or justifying a maintenance reserve draw. On consistency, you get the same crew and the same quality whether we're on Unit 2B this week or Unit 17 next month, so you're not re-vetting a new contractor every turnover.",
      "We work with independent landlords owning one duplex, with property management companies running 50+ unit portfolios, and with real estate agents prepping listings across Bergen County. Typical jobs include tenant-damage repair during turnovers, listing-prep patch-and-paint, hallway and common-area drywall after delivery damage or accidental impact, recurring maintenance on older buildings with settling cracks, and coordinating with plumbers or roofers on water-damage repairs. For clients with 5+ units, we offer portfolio pricing and quarterly billing. For one-off landlords, you get the same speed and documentation at standard rates.",
    ],
    processSteps: [
      { title: "Portfolio Intake (New Clients)", description: "For property managers and landlords new to us, we do a 15-minute call to understand your portfolio size, typical turnover cadence, and documentation needs. We set up a billing profile, share our COI, and confirm your preferred contact method." },
      { title: "Turnover or Service Request", description: "You text, email, or call with the unit address, access details, and scope. We confirm same-week availability and give you a fixed-price estimate before any work begins — no surprise billing." },
      { title: "On-Site Access & Execution", description: "We coordinate key pickup, lockbox access, or scheduled entry with your office. We carry drywall, compound, texture, and primer on the truck so most jobs finish in one visit." },
      { title: "Repair to Move-In Ready Standard", description: "Holes cut and patched, anchor craters filled, impact damage feathered out, texture matched, primed, and painted with a clean white or your portfolio's standard paint color. Trim, floors, and appliances protected throughout." },
      { title: "Documentation Packet", description: "Every job closes with an itemized invoice (labor and materials broken out by room), before/after photos, and a written scope of work suitable for security deposit deductions, adjuster reports, or owner statements." },
      { title: "Billing & Follow-Up", description: "Invoice by email with Net-15 or Net-30 terms for established clients. Quarterly portfolio billing available for 5+ unit clients. Any warranty questions addressed within 48 hours for 90 days post-service." },
    ],
    commonIssues: [
      { problem: "Tenant-caused damage during move-out (holes, anchor craters, impact dents)", solution: "We patch every hole, fill every anchor crater, feather out impact dents, texture-match, and repaint to move-in-ready condition. Itemized invoice and photos included — everything you need to deduct against the security deposit." },
      { problem: "Pre-listing patch-and-paint for realtors and sellers", solution: "Fast turnaround (often same-week) on holes, cracks, and scuffs before Zillow photos or showings. We work around showings and coordinate directly with you or the seller to minimize access friction." },
      { problem: "Multi-unit common area damage (hallways, entryways, laundry rooms)", solution: "We handle high-traffic area repairs — delivery damage, tenant move-in scrapes, accidental impacts — on a schedule that doesn't disrupt other tenants. Evening or weekend scheduling available." },
      { problem: "Recurring settling cracks in older buildings", solution: "For pre-war and mid-century buildings with chronic settling, we use flexible tape and three-coat compound to minimize return-crack rate. We can also set up quarterly or semi-annual maintenance visits for portfolios with persistent patterns." },
      { problem: "Water damage repairs coordinated with plumbers, roofers, or HVAC contractors", solution: "We coordinate directly with the trade contractor on the root cause, wait for the space to dry, and handle the drywall portion cleanly — including insurance documentation if a claim is involved." },
      { problem: "Volume pricing for 5+ unit landlords and property management companies", solution: "Established portfolio clients get standing pricing, priority scheduling during peak turnover seasons, and quarterly billing terms. We provide references from similar-scale clients on request." },
    ],
    pricingInfo: {
      range: "Typical turnover drywall work in Bergen County runs $200–$900 per unit; portfolio clients receive standing rates",
      factors: ["Number and size of holes, craters, and dents per unit", "Whether full-room repaint or spot touch-up is included", "Ceiling repairs vs. wall repairs", "Common area vs. individual unit work", "Portfolio volume (5+ units qualifies for standing rates)"],
      note: "Itemized invoices standard. Net-15 or Net-30 billing for established clients. Quarterly portfolio billing available for 5+ unit landlords and property management companies.",
    },
    serviceFAQs: [
      { q: "Do you offer portfolio pricing for landlords and property managers?", a: "Yes. Clients with 5+ units under management get standing rates (published on your profile, no quote-per-job needed for routine work), priority scheduling during peak turnover seasons, and quarterly billing terms. Smaller portfolios get the same speed and documentation at standard rates." },
      { q: "How fast can you turn a unit?", a: "For most tenant-damage turnovers in Bergen County, we schedule within the same week and complete the drywall portion in one or two visits (depending on whether painting is included and how much compound drying time is needed). We hold turnover slots open for portfolio clients specifically to beat the typical contractor waitlist." },
      { q: "Do you provide documentation for security deposit deductions?", a: "Yes. Every invoice is itemized by room with labor and materials broken out, and we include before/during/after photos in the packet. That's the standard documentation format landlord-tenant courts accept and that most states' security-deposit statutes expect." },
      { q: "Can you coordinate access through a lockbox or leasing office?", a: "Yes. We're used to coordinating with leasing offices, lockboxes, key exchanges, and scheduled tenant access. Just share the access protocol when you book the job and we'll work within it." },
      { q: "Do you carry a certificate of insurance (COI) we can keep on file?", a: "Yes. We carry full general liability and workers' comp insurance and can provide a COI naming your property management company or ownership entity as additional insured if your properties require it. Most established portfolio clients have our COI on file." },
      { q: "Do you handle commercial or mixed-use buildings in addition to residential rentals?", a: "Yes, for tenant-space drywall repair in small commercial (office, retail, medical) and mixed-use buildings across Bergen County. Larger ground-up commercial drywall installation is a different scope — call us to discuss whether we're the right fit." },
    ],
  },
  {
    slug: "ceiling-repair",
    title: "Ceiling Repair",
    shortDesc: "Fix cracks, water stains, sagging, and damage to restore your ceiling's appearance.",
    longDesc: "Ceiling damage can be caused by water leaks, settling, or age. Our team expertly repairs cracks, water stains, sagging sections, and other ceiling issues. We ensure a smooth, seamless finish that blends perfectly with your existing ceiling.",
    icon: "🏠",
    benefits: ["Water stain removal", "Crack and seam repair", "Sagging ceiling fix", "Seamless blending"],
    keywords: ["ceiling repair", "ceiling crack repair", "water stain ceiling", "ceiling patching"],
    introParagraphs: [
      "Ceiling damage is one of the most stressful repairs a homeowner faces. It's visible from everywhere in the room, it often signals a bigger problem like a plumbing leak or ice-dam damage, and nobody enjoys working overhead on a ladder for hours. The Patch Boys of Bergen County repairs ceilings every single week — water stains from second-floor bathrooms, cracks along seams, sagging panels, and textured ceilings that need patching after a leak has been fixed. We know Bergen County's housing stock well, from prewar colonials in Ridgewood and Glen Rock to newer construction in Mahwah and Montvale, and we adjust our approach to each ceiling type.",
      "A ceiling repair isn't just a drywall repair turned sideways. Gravity works against you, the compound has to be applied in thinner coats, texture matching is harder because ceilings are lit differently than walls, and any mistake is visible from every angle. Our technicians have repaired thousands of ceilings across Bergen County and carry specialized ladders, stilts, spray rigs, and lighting so the finished repair blends cleanly with the surrounding surface.",
      "Before we patch, we identify what caused the damage. A water stain without a fixed leak will come back. A sagging section usually means wet insulation or compromised fasteners. A recurring crack along a seam often means the tape has failed and needs to be replaced, not just re-mudded. We take the time to diagnose correctly so the repair actually lasts.",
    ],
    processSteps: [
      { title: "Inspection & Diagnosis", description: "We locate the source of the damage — leak, settling, failed tape, or impact — and confirm it's resolved before starting cosmetic repair." },
      { title: "Protection", description: "Ceiling work is messy. We cover floors and furniture with plastic and drop cloths, and we mask off light fixtures and vents." },
      { title: "Damage Removal", description: "We cut back wet, stained, or compromised drywall to solid material. For sagging sections we remove the panel back to the ceiling joists." },
      { title: "New Drywall or Patch Install", description: "We install new drywall or a patch panel, securing it to joists with screws and backing as needed." },
      { title: "Tape, Mud, Sand", description: "We apply tape and three coats of compound, sanding between coats for a flat, feathered transition to the existing ceiling." },
      { title: "Texture Match & Paint", description: "We match the existing ceiling texture — smooth, stomp, swirl, orange peel, or knockdown — then prime and paint to blend the repair invisibly." },
    ],
    commonIssues: [
      { problem: "Brown water stains from upstairs leaks", solution: "Once the leak is fixed, we cut out the stained drywall, inspect for mold and wet insulation, and install fresh sheetrock with stain-blocking primer." },
      { problem: "Cracks along the drywall-to-ceiling seam", solution: "We remove the failed tape, re-tape the seam, and apply three coats of compound to stop the crack from returning." },
      { problem: "Sagging ceiling panels", solution: "We remove the sagging section back to joists, check for water damage or failed fasteners, and install new drywall with proper screw spacing." },
      { problem: "Nail pops and visible fastener heads", solution: "We reset the fasteners, drive new screws, and finish with three coats of compound and matching texture." },
      { problem: "Cracks radiating from light fixtures or HVAC vents", solution: "We re-secure the drywall around the fixture, tape and mud the cracks, and texture-match to blend the repair." },
      { problem: "Damage from ice dams or roof leaks", solution: "After the roof issue is fixed, we replace water-damaged drywall, treat for any mold concerns, and restore texture and paint." },
    ],
    pricingInfo: {
      range: "Most ceiling repairs in Bergen County run $200–$700",
      factors: ["Size of the damaged area", "Ceiling height (standard 8-foot vs. vaulted or cathedral)", "Texture type and complexity", "Whether water damage remediation is needed", "Painting and priming inclusion"],
      note: "We provide free, written estimates for every ceiling repair — no hidden fees, no day-of surprises.",
    },
    serviceFAQs: [
      { q: "My ceiling has a water stain — do I need to fix the leak first?", a: "Yes. We can help you identify the source, but the leak must be repaired (by a plumber or roofer) before we install new drywall. Otherwise the stain will return." },
      { q: "Can you match a textured ceiling?", a: "Yes. We match all common ceiling textures including smooth, stomp, swirl, orange peel, and knockdown. Our technicians carry the equipment needed to replicate each texture." },
      { q: "How long does a ceiling repair take?", a: "Small cosmetic repairs take 2–4 hours. Larger repairs involving water damage or multiple panels may require two visits to allow compound to dry fully between coats." },
      { q: "Do you repair popcorn ceilings?", a: "Yes, we patch popcorn ceilings and can also remove them entirely if you'd prefer a modern smooth finish. See our popcorn ceiling removal service for details." },
    ],
  },
  {
    slug: "plaster-repair",
    title: "Plaster Repair",
    shortDesc: "Specialized repair for older plaster walls and ceilings common in Bergen County homes.",
    longDesc: "Many Bergen County homes feature original plaster walls that require specialized repair techniques. Our technicians are trained in traditional plaster repair methods, preserving the character of your home while restoring damaged areas to their former beauty.",
    icon: "🏗️",
    benefits: ["Historic home specialists", "Traditional methods", "Crack and crumble repair", "Preserves home character"],
    keywords: ["plaster repair", "plaster wall repair", "plaster ceiling repair", "old house plaster"],
    introParagraphs: [
      "Bergen County is full of beautiful older homes — prewar colonials, Tudors, Victorians, and midcentury capes — and a huge number of them still have original plaster walls and ceilings. Plaster is denser, more sound-absorbent, and more fire-resistant than drywall, and it's a big part of what gives these homes their character. But plaster also cracks, crumbles, delaminates from the lath, and develops spider-web fractures as houses settle. Fixing plaster correctly requires different materials, different tools, and different techniques than drywall — which is why most general handymen leave plaster repairs looking worse than when they started.",
      "The Patch Boys of Bergen County repairs plaster the right way. We use setting-type compounds, plaster washers, bonding agents, and traditional three-coat methods when appropriate, and we know when a repair should be re-plastered versus skim-coated versus patched with drywall. We've worked on homes in Ridgewood, Englewood, Teaneck, Rutherford, Hackensack, and throughout Bergen County that are 80, 100, even 120 years old — and we treat them with the care they deserve.",
      "We also understand the realities of living in an old house: you don't want to tear out a whole wall to fix a crack, and you shouldn't have to. Most plaster repairs can be done in a way that preserves the original surrounding material while making the damaged area disappear. That's our specialty.",
    ],
    processSteps: [
      { title: "Assess the Plaster System", description: "We identify whether you have traditional lath-and-plaster, rock lath, or plaster over drywall — each requires a different repair approach." },
      { title: "Stabilize Loose Plaster", description: "We use plaster washers and adhesive to reattach plaster that has separated from the lath, preventing further crumbling." },
      { title: "Remove Damaged Material", description: "We carefully remove only the crumbled or cracked plaster, preserving as much of the original surface as possible." },
      { title: "Bonding & Base Coat", description: "We apply a bonding agent to the lath or substrate, then a base coat of setting-type compound to build the repair to level." },
      { title: "Finish Coat & Blending", description: "We apply a finish coat, feather the edges into the surrounding plaster, and sand for a seamless transition." },
      { title: "Texture Match & Paint", description: "We replicate any hand-troweled, sand, or skip-trowel texture in the original plaster and touch up the paint so the repair disappears." },
    ],
    commonIssues: [
      { problem: "Spider-web cracks from house settling", solution: "We stabilize the area, apply fiberglass mesh if needed, and skim-coat with setting-type compound to stop crack recurrence." },
      { problem: "Plaster separating from wood lath", solution: "We install plaster washers to re-anchor the plaster to the lath, then skim-coat over the washers for an invisible finish." },
      { problem: "Crumbled sections from water damage", solution: "After the leak is resolved, we cut back to sound plaster, apply bonding agent, and rebuild the area with base coat and finish coat plaster." },
      { problem: "Cracks around doorways and windows", solution: "We re-tape the seam with mesh tape, apply setting-type compound in three coats, and blend into the surrounding texture." },
      { problem: "Ceiling plaster sagging or detaching", solution: "We stabilize with washers where possible, or remove and replace the compromised area with matching material." },
      { problem: "Patches from old renovations that never blended", solution: "We skim-coat the entire patched area, matching texture and feathering edges so previous bad repairs disappear." },
    ],
    pricingInfo: {
      range: "Most plaster repairs in Bergen County run $250–$900",
      factors: ["Severity and size of damage", "Whether stabilization (plaster washers) is needed", "Texture complexity of the original plaster", "Ceiling vs. wall repairs", "Access and working height"],
      note: "Plaster work requires more time and specialized materials than drywall, but we'll always give you a fixed, written estimate before starting.",
    },
    serviceFAQs: [
      { q: "Should I replace my plaster walls with drywall?", a: "Usually no. Plaster is a valuable feature of older homes and removing it is expensive and messy. Almost all plaster problems can be repaired while preserving the original walls." },
      { q: "Can you match the texture of my old plaster?", a: "Yes. We replicate sand, skip-trowel, hand-troweled, and smooth plaster finishes so the repaired area blends with the surrounding surface." },
      { q: "My plaster is cracking in many places. Do I need to redo the whole wall?", a: "Not necessarily. Many older homes have widespread hairline cracks that can be skim-coated rather than fully replaced. We'll evaluate and give you honest options." },
      { q: "Do you use traditional lime plaster or modern compound?", a: "We typically use modern setting-type joint compounds designed specifically for plaster repair, which bond strongly and finish smoothly. For historic preservation projects we can discuss traditional materials." },
    ],
  },
  {
    slug: "drywall-installation",
    title: "Drywall Installation",
    shortDesc: "Professional new drywall installation for renovations, additions, and new construction.",
    longDesc: "Need new drywall hung? Whether you're finishing a basement, renovating a room, or building an addition, our team delivers precise drywall installation with clean cuts, tight seams, and professional finishing. We handle projects of all sizes throughout Bergen County.",
    icon: "📐",
    benefits: ["Basement finishing", "Room renovations", "Additions & new construction", "Professional finishing"],
    keywords: ["drywall installation", "drywall hanging", "sheetrock installation", "new drywall"],
    introParagraphs: [
      "Whether you're finishing a Bergen County basement, renovating a kitchen, building an addition, or converting an attic, new drywall installation sets the stage for every finish that comes after it. Walls that are out of plane, seams that telegraph through paint, and outside corners that chip or crack can ruin an otherwise beautiful renovation. The Patch Boys of Bergen County installs drywall the way good contractors expect: square, plumb, tight-seamed, and finished to a Level 4 or Level 5 smoothness depending on the job.",
      "We handle installations of every size, from a single wall replacement to finishing an entire basement or new addition. We hang standard 1/2-inch drywall, 5/8-inch Type X fire-rated drywall where code requires it, moisture-resistant green board for bathrooms and laundry rooms, and cement board for wet areas around tubs and showers. Every panel is fastened to manufacturer spacing, every seam is taped and three-coated, and every outside corner gets metal corner bead for durability.",
      "Beyond the installation itself, we coordinate with the other trades on your project — electricians, plumbers, HVAC — to make sure rough-ins are complete and inspections have passed before we hang. And when we're done, you have a flat, paint-ready surface that lets your painter, trim carpenter, or cabinet installer do their best work.",
    ],
    processSteps: [
      { title: "Site Measurement & Quote", description: "We measure your space, calculate square footage and panel count, and give you a fixed-price estimate including materials, labor, and finishing level." },
      { title: "Material Delivery & Prep", description: "We deliver drywall, compound, tape, corner bead, and screws to your site and protect flooring and any existing finishes." },
      { title: "Hanging the Panels", description: "We hang drywall horizontally on walls and perpendicular to joists on ceilings, fastening with screws at proper spacing and keeping seams tight." },
      { title: "Corner Bead Installation", description: "We install metal or paper-faced corner bead on every outside corner to provide a clean, durable edge that resists chipping." },
      { title: "Taping & Three-Coat Finish", description: "We tape every seam and inside corner, then apply three coats of joint compound, sanding between coats for a flat, feathered finish." },
      { title: "Final Sanding & Walk-Through", description: "We sand the final coat to Level 4 (or Level 5 for critical lighting), vacuum the space, and walk through with you to confirm the surface is paint-ready." },
    ],
    commonIssues: [
      { problem: "Basement finishing with moisture concerns", solution: "We use moisture-resistant drywall in below-grade spaces, proper vapor barriers, and leave air gaps behind the drywall where recommended." },
      { problem: "Ceilings that need to hide beams and ductwork", solution: "We frame soffits and furr-downs, then drywall to create clean lines that disguise mechanicals while meeting clearance requirements." },
      { problem: "Fire code requirements around garages and between units", solution: "We install 5/8-inch Type X fire-rated drywall wherever code requires it, with properly taped seams for the full fire rating." },
      { problem: "Bathrooms, laundry rooms, and wet areas", solution: "We use green board moisture-resistant drywall for standard wet rooms and cement backer board behind tile in tub and shower surrounds." },
      { problem: "High-end rooms with critical lighting", solution: "We deliver a Level 5 finish — a full skim coat over the entire surface — so raking light won't reveal seams or fastener heads." },
      { problem: "Matching into existing drywall on renovations", solution: "We feather new seams into existing surfaces, tape carefully, and texture-match so old and new drywall blend invisibly." },
    ],
    pricingInfo: {
      range: "Drywall installation in Bergen County typically runs $2–$4 per square foot installed and finished",
      factors: ["Square footage and ceiling height", "Drywall type (standard, fire-rated, moisture-resistant, cement board)", "Finishing level (Level 3, 4, or 5)", "Number of corners, openings, and cutouts", "Accessibility and project complexity"],
      note: "Every installation starts with a free, detailed written estimate so you know exactly what's included.",
    },
    serviceFAQs: [
      { q: "Do you handle whole-room or whole-basement installations?", a: "Yes. We regularly finish entire basements, additions, and renovated rooms throughout Bergen County, from framing-ready to paint-ready." },
      { q: "What finishing level do you deliver?", a: "We deliver a Level 4 finish by default, which is the standard for most walls and ceilings. For rooms with critical lighting, we offer Level 5 (full skim coat) for an additional charge." },
      { q: "Do you haul away the debris?", a: "Yes. We include cleanup and debris removal in every installation quote — we don't leave drywall scraps or compound buckets behind." },
      { q: "Can you coordinate with my electrician or plumber?", a: "Absolutely. We regularly work alongside other trades and can schedule our hang to happen right after electrical, plumbing, and HVAC rough-ins pass inspection." },
    ],
  },
  {
    slug: "popcorn-ceiling-removal",
    title: "Popcorn Ceiling Removal",
    shortDesc: "Modernize your home by removing outdated popcorn texture from your ceilings.",
    longDesc: "Popcorn ceilings are outdated and can even contain asbestos in older homes. Our team safely removes popcorn ceiling texture and replaces it with a smooth, modern finish that instantly updates the look of any room and can increase your home's value.",
    icon: "✨",
    benefits: ["Safe removal process", "Modern smooth finish", "Increases home value", "Asbestos-aware procedures"],
    keywords: ["popcorn ceiling removal", "popcorn ceiling", "textured ceiling removal", "ceiling modernization"],
    introParagraphs: [
      "If your Bergen County home was built between the 1950s and the late 1980s, there's a good chance it has popcorn ceilings — also called acoustic or cottage-cheese texture — in at least one room. Popcorn was popular because it was cheap to spray, it hid imperfections, and it absorbed sound. Today, it's widely considered dated, it collects dust and cobwebs, and it can drag down the perceived value of an otherwise updated home. Removing popcorn ceilings and replacing them with a smooth modern finish is one of the single best cosmetic upgrades a homeowner can make.",
      "The Patch Boys of Bergen County removes popcorn ceilings safely and cleanly. We protect your floors, walls, and furniture with heavy plastic and drop cloths, mist the texture with water to soften it, scrape it off in controlled sheets, and then skim-coat the ceiling to a smooth finish before priming and painting. The transformation is dramatic — rooms feel taller, brighter, and noticeably more modern.",
      "We also take asbestos seriously. Popcorn texture applied before 1980 may contain asbestos, and any removal should begin with a test of the material. We coordinate asbestos testing with a certified lab and advise you on the right path forward. If the material tests positive, we'll refer you to a licensed abatement contractor; if it's negative, we can remove it ourselves as part of a complete ceiling refresh.",
    ],
    processSteps: [
      { title: "Asbestos Testing (Pre-1980 Homes)", description: "We send a sample to a certified lab and wait for results before beginning any removal on homes that may have asbestos-containing texture." },
      { title: "Room Prep & Containment", description: "We cover floors, walls, outlets, vents, and furniture with plastic sheeting and drop cloths to contain dust and water." },
      { title: "Wet & Scrape", description: "We mist the ceiling with water, let it soak in, and scrape off the softened texture with wide drywall knives." },
      { title: "Ceiling Repair", description: "After scraping, we repair any gouges, failed tape, or nail pops revealed during removal so the substrate is smooth." },
      { title: "Skim Coat", description: "We apply one or two skim coats of joint compound over the entire ceiling and sand smooth for a flat, modern surface." },
      { title: "Prime & Paint", description: "We apply a quality primer and finish coat of ceiling paint for a clean, bright, and updated look that instantly modernizes the room." },
    ],
    commonIssues: [
      { problem: "Asbestos-containing texture in pre-1980 homes", solution: "We arrange testing and, if positive, refer you to a licensed abatement contractor. If negative, we proceed with standard removal." },
      { problem: "Ceilings that have been painted over", solution: "Painted popcorn is harder to remove because water doesn't soak in. We may need to scrape dry or skim-coat directly over the texture." },
      { problem: "Damaged drywall revealed after removal", solution: "We repair any gouges, failed tape, or water stains as part of the process before skim-coating." },
      { problem: "Textured walls adjacent to popcorn ceilings", solution: "We mask and protect textured walls, then carefully feather the ceiling edge so the transition looks clean." },
      { problem: "Dust and debris control", solution: "We contain the work area with plastic sheeting and clean thoroughly at the end of every visit to keep the rest of your home dust-free." },
      { problem: "Large open-concept ceilings", solution: "We scope the job to minimize disruption, often working room-by-room so you can continue to use the rest of your home." },
    ],
    pricingInfo: {
      range: "Popcorn ceiling removal in Bergen County typically runs $2.50–$5.00 per square foot",
      factors: ["Room square footage and ceiling height", "Whether the popcorn has been painted over", "Asbestos testing results", "Condition of the underlying drywall", "Number of rooms and project scope"],
      note: "We provide a free, fixed-price quote once we've tested for asbestos and measured the space.",
    },
    serviceFAQs: [
      { q: "Does my popcorn ceiling contain asbestos?", a: "Popcorn applied before 1980 may contain asbestos. We recommend testing before removal. If your home was built after 1980, it's very unlikely to contain asbestos." },
      { q: "How long does popcorn removal take?", a: "Most single-room removals take 1–2 days, including skim coating and paint. Larger homes with multiple rooms may take 3–5 days." },
      { q: "Is the process messy?", a: "It's a wet, dusty process, but we contain the work area thoroughly with plastic sheeting and clean up completely at the end. Your floors, furniture, and walls are fully protected." },
      { q: "Will removing popcorn increase my home's value?", a: "It typically does. Popcorn ceilings are widely viewed as dated, and buyers often see removal as a task they'd rather not take on themselves." },
    ],
  },
  {
    slug: "texture-matching",
    title: "Texture Matching",
    shortDesc: "Seamless texture matching so your repairs are completely invisible.",
    longDesc: "The hallmark of a great drywall repair is invisible results. Our texture matching experts analyze your existing wall or ceiling texture and replicate it precisely, ensuring that repaired areas blend seamlessly with the surrounding surface. From orange peel to knockdown, we match it all.",
    icon: "🎨",
    benefits: ["Invisible repair results", "All texture types matched", "Orange peel & knockdown", "Expert color matching"],
    keywords: ["texture matching", "wall texture matching", "drywall texture", "knockdown texture", "orange peel texture"],
    introParagraphs: [
      "Texture matching is the difference between a drywall repair you can see and one you can't. A patched hole with the wrong texture is as obvious as no patch at all — sometimes worse, because now there's a smooth island surrounded by rough wall. The Patch Boys of Bergen County has built its reputation on invisible repairs, and texture matching is the skill that makes invisibility possible. Our technicians have hand-matched every common wall and ceiling texture used in Bergen County homes for decades, and we carry the equipment needed to replicate each one on site.",
      "The most common textures we match are orange peel, knockdown, and smooth (Level 4 or Level 5) — these are standard in homes built from the 1980s onward. We also match stomp and swirl ceiling textures, sand and skip-trowel finishes common in older plaster homes, and hand-troweled custom textures that were applied by the original drywaller. Every texture has its own technique, its own tools, and its own timing, and we know them all.",
      "Texture matching also includes paint blending, which is its own art. Even if the texture is perfect, a repair will stand out if the paint doesn't match — sheen differences, color fade, and touch-up technique all matter. We prime repairs correctly, feather paint into surrounding areas, and use the right roller nap for the existing texture so the repair disappears under normal lighting.",
    ],
    processSteps: [
      { title: "Texture Analysis", description: "We examine the existing texture closely — pattern, depth, spacing, and technique — to identify exactly what we're matching." },
      { title: "Practice Board", description: "For complex or hand-applied textures, we test on a practice board first to dial in the technique before touching your wall." },
      { title: "Surface Prep", description: "We ensure the underlying compound is smooth and properly primed so the texture application will behave the same as the surrounding surface." },
      { title: "Texture Application", description: "We spray, roll, stomp, or hand-trowel the texture using the method that matches your existing finish." },
      { title: "Edge Blending", description: "We feather the texture edges into the surrounding surface so there's no visible transition line." },
      { title: "Paint Match & Touch-Up", description: "We prime the textured area and blend paint using a matched color, sheen, and application technique for a truly invisible finish." },
    ],
    commonIssues: [
      { problem: "Orange peel walls with light splatter", solution: "We use a texture hopper gun with fine-tip nozzle to spray a matching splatter pattern, then prime and paint." },
      { problem: "Knockdown texture on walls and ceilings", solution: "We spray a heavier splatter, let it set briefly, and knock it down with a wide drywall knife to match the original flattened peaks." },
      { problem: "Smooth Level 5 walls in high-end rooms", solution: "We skim-coat the repair, sand progressively finer, and feather wide so raking light doesn't reveal the patch." },
      { problem: "Stomp, swirl, or rosebud ceiling texture", solution: "We apply thick joint compound and stamp the pattern with a sponge, brush, or trowel to match the original ceiling." },
      { problem: "Hand-troweled sand and skip-trowel plaster", solution: "We hand-apply setting-type compound with a trowel, replicating the sweep and rhythm of the original finish." },
      { problem: "Custom or one-of-a-kind textures", solution: "We analyze the original, test on a practice board, and replicate the technique until the match is indistinguishable." },
    ],
    pricingInfo: {
      range: "Texture matching is typically included in drywall and ceiling repair pricing",
      factors: ["Complexity of the original texture", "Whether we need a practice board to dial in the technique", "Size of the repair area", "Whether custom or hand-applied texture is required"],
      note: "Texture matching is part of our repair process on every drywall and ceiling job — we don't charge extra unless the texture is highly custom or labor-intensive.",
    },
    serviceFAQs: [
      { q: "Can you match any texture?", a: "We match every common texture used in Bergen County homes — orange peel, knockdown, smooth, stomp, swirl, sand, skip-trowel, and hand-troweled. If you have a highly custom texture, we can test-match on a practice board first." },
      { q: "How close can you get to the original?", a: "Under normal lighting, our texture matches are invisible. Raking light can reveal any imperfection, which is why we offer Level 5 smooth finishes for rooms with critical lighting conditions." },
      { q: "Do you match the paint too?", a: "Yes. Texture matching and paint matching go together — we prime the repair, blend paint color and sheen, and feather the touch-up into the surrounding wall." },
      { q: "I've had a bad repair done before. Can you fix it?", a: "Yes. We regularly re-do repairs that other contractors left visible. We'll skim-coat the area, re-texture, and blend paint to make the original patch disappear." },
    ],
  },
  {
    slug: "painting",
    title: "Painting & Touch-Up",
    shortDesc: "Complete your repair with professional painting and color matching for a flawless finish.",
    longDesc: "Why hire two contractors when one can do it all? After repairing your drywall, we offer professional painting and touch-up services to complete the job from start to finish. Our color matching ensures a seamless blend with your existing paint.",
    icon: "🖌️",
    benefits: ["One contractor, complete job", "Expert color matching", "Interior touch-ups", "Full room painting available"],
    keywords: ["painting after drywall repair", "paint matching", "interior painting", "touch-up painting"],
    introParagraphs: [
      "A drywall repair isn't really finished until it's painted. Raw joint compound is porous and bright white, and skipping the paint step leaves an obvious patch — especially on colored walls. The Patch Boys of Bergen County offers painting and touch-up services as part of every repair, so you get a truly complete result from one contractor in a single visit. No coordinating a separate painter, no waiting days between appointments, no worrying about who's responsible for the final look.",
      "We handle everything from small touch-ups around patched repairs to full-room repaints for Bergen County homeowners who want to refresh the whole space while the drop cloths are already down. We color-match your existing paint on site when you don't have the original can, we apply the right primer for the substrate, and we use the correct roller nap and application technique to match the surrounding wall texture and sheen.",
      "Paint is where most DIY repairs go wrong — fresh paint applied over a repair flashes differently than the surrounding wall, sheens don't match, and the patch stands out even when the drywall work was good. We solve that by treating paint as the final, most important step of the repair, not an afterthought.",
    ],
    processSteps: [
      { title: "Color & Sheen Match", description: "We match your existing paint color and sheen — either from your original can or by matching on-site with a handheld color reader." },
      { title: "Primer Application", description: "We apply a stain-blocking or drywall primer to the repair area so the finish coat absorbs evenly and the patch doesn't flash." },
      { title: "Feathered Touch-Up", description: "For small repairs, we roll paint onto the patch and feather it into the surrounding wall so the edge disappears." },
      { title: "Full-Wall or Room Repaint", description: "For larger repairs or when touch-ups would be too visible, we repaint the full wall or room for a uniform finish." },
      { title: "Trim & Cut-Ins", description: "We cut in cleanly along trim, ceilings, and corners, protecting adjacent surfaces with tape and drop cloths." },
      { title: "Cleanup & Walk-Through", description: "We remove all protection, clean our tools and workspace, and walk through with you to confirm the finish is exactly what you expected." },
    ],
    commonIssues: [
      { problem: "Patches that flash differently than the wall", solution: "We prime the repair correctly and, where needed, repaint the full wall corner-to-corner so the sheen is uniform." },
      { problem: "Paint color that's faded or no longer available", solution: "We match on-site using a color reader, or we recommend repainting the full wall with a close modern equivalent." },
      { problem: "Mismatched sheen between touch-up and original", solution: "We confirm the original sheen (flat, eggshell, satin, semi-gloss) and apply the matching sheen for a uniform final look." },
      { problem: "Water stains bleeding through new paint", solution: "We use stain-blocking primer designed to seal water damage so stains don't ghost through the finish coat." },
      { problem: "Popping nail heads and screws after paint", solution: "We reset fasteners, spot-prime, and feather touch-up paint so the repair doesn't show." },
      { problem: "Rooms that need a full repaint while we're on site", solution: "We can expand the scope to repaint a full wall, a single room, or an entire floor while drop cloths and protection are already in place." },
    ],
    pricingInfo: {
      range: "Touch-up paint is typically included in repair pricing; full-room painting runs $300–$900 per room",
      factors: ["Scope (spot touch-up vs. full wall vs. full room)", "Ceiling height and wall square footage", "Color changes vs. same-color repaint", "Trim, doors, and ceilings included", "Wall condition and prep required"],
      note: "We include reasonable touch-up paint with every repair at no extra charge. Full-room and whole-house painting are quoted separately.",
    },
    serviceFAQs: [
      { q: "Do you include painting with drywall repairs?", a: "Yes — basic touch-up painting is included with most of our drywall and ceiling repairs so you get a complete result from one contractor." },
      { q: "Can you match my existing paint color?", a: "Yes. We can match on-site using a handheld color reader, or we can use your original paint if you still have some. For older faded paint, we may recommend a full-wall repaint for the cleanest look." },
      { q: "Will the touch-up be visible?", a: "Properly executed touch-ups are invisible under normal lighting. For high-visibility rooms or walls with aged paint, we sometimes recommend repainting the full wall to guarantee a uniform finish." },
      { q: "Do you do whole-room or whole-house painting?", a: "Yes. We regularly repaint full rooms and entire homes throughout Bergen County, often in combination with drywall and ceiling repair work." },
    ],
  },
];

export const BERGEN_TOWNS = [
  { name: "Hackensack", slug: "hackensack", zip: "07601", population: "44,522" },
  { name: "Paramus", slug: "paramus", zip: "07652", population: "26,342" },
  { name: "Teaneck", slug: "teaneck", zip: "07666", population: "40,356" },
  { name: "Fort Lee", slug: "fort-lee", zip: "07024", population: "37,907" },
  { name: "Englewood", slug: "englewood", zip: "07631", population: "28,390" },
  { name: "Ridgewood", slug: "ridgewood", zip: "07450", population: "25,979" },
  { name: "Bergenfield", slug: "bergenfield", zip: "07621", population: "27,375" },
  { name: "Fair Lawn", slug: "fair-lawn", zip: "07410", population: "33,710" },
  { name: "Garfield", slug: "garfield", zip: "07026", population: "32,304" },
  { name: "Lodi", slug: "lodi", zip: "07644", population: "24,842" },
  { name: "Mahwah", slug: "mahwah", zip: "07430", population: "26,363" },
  { name: "Ramsey", slug: "ramsey", zip: "07446", population: "14,893" },
  { name: "Wyckoff", slug: "wyckoff", zip: "07481", population: "17,006" },
  { name: "Glen Rock", slug: "glen-rock", zip: "07452", population: "11,926" },
  { name: "Rutherford", slug: "rutherford", zip: "07070", population: "18,858" },
  { name: "Lyndhurst", slug: "lyndhurst", zip: "07071", population: "22,324" },
  { name: "Saddle Brook", slug: "saddle-brook", zip: "07663", population: "13,628" },
  { name: "River Edge", slug: "river-edge", zip: "07661", population: "11,688" },
  { name: "Closter", slug: "closter", zip: "07624", population: "8,731" },
  { name: "Westwood", slug: "westwood", zip: "07675", population: "11,325" },
  { name: "Dumont", slug: "dumont", zip: "07628", population: "18,140" },
  { name: "New Milford", slug: "new-milford", zip: "07646", population: "16,477" },
  { name: "Oradell", slug: "oradell", zip: "07649", population: "8,164" },
  { name: "Waldwick", slug: "waldwick", zip: "07463", population: "10,102" },
  { name: "Upper Saddle River", slug: "upper-saddle-river", zip: "07458", population: "8,208" },
  { name: "Allendale", slug: "allendale", zip: "07401", population: "6,848" },
  { name: "Cresskill", slug: "cresskill", zip: "07626", population: "9,069" },
  { name: "Demarest", slug: "demarest", zip: "07627", population: "4,985" },
  { name: "Emerson", slug: "emerson", zip: "07630", population: "7,768" },
  { name: "Hillsdale", slug: "hillsdale", zip: "07642", population: "10,219" },
  { name: "Montvale", slug: "montvale", zip: "07645", population: "8,632" },
  { name: "Park Ridge", slug: "park-ridge", zip: "07656", population: "9,019" },
  { name: "Washington Township", slug: "washington-township", zip: "07676", population: "9,721" },
  { name: "Woodcliff Lake", slug: "woodcliff-lake", zip: "07677", population: "5,998" },
  { name: "Haworth", slug: "haworth", zip: "07641", population: "3,509" },
  { name: "Old Tappan", slug: "old-tappan", zip: "07675", population: "5,983" },
  { name: "Northvale", slug: "northvale", zip: "07647", population: "4,861" },
  { name: "Norwood", slug: "norwood", zip: "07648", population: "5,793" },
];

export const TESTIMONIALS = [
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "Fair price given the quality of work, and the excellent customer service. Dan was great to work with!",
    rating: 5,
    source: "HomeAdvisor",
  },
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "I would definitely recommend this company. They did an excellent job of patching what needed to be patched and then matching the texture on our ceiling.",
    rating: 5,
    source: "HomeAdvisor",
  },
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "They repaired our dining room plaster ceilings and bedroom walls expertly. The work was done with care and precision.",
    rating: 5,
    source: "HomeAdvisor",
  },
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "Patch Boys was a lifesaver in helping me out with my repair. They were super responsive and did an amazing job. Highly recommend!",
    rating: 5,
    source: "Thumbtack",
  },
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "Professional, on time, and left the place clean. The patching work was seamless — you can't even tell where the damage was. Will use again!",
    rating: 5,
    source: "Google",
  },
  {
    name: "Verified Customer",
    location: "Bergen County, NJ",
    text: "Excellent service from start to finish. Got a free estimate, fair pricing, and the repair was completed in the same visit. Top-notch work.",
    rating: 5,
    source: "Google",
  },
];

export const FAQ = [
  {
    q: "How much does drywall repair cost?",
    a: "Most drywall repairs range from $150 to $500 depending on the size and complexity. We provide free, no-obligation estimates so you'll know the exact cost upfront before any work begins.",
  },
  {
    q: "Do you offer free estimates?",
    a: "Yes! We offer completely free, no-obligation estimates for all drywall repair and installation services throughout Bergen County. Call us or fill out our contact form to schedule yours.",
  },
  {
    q: "How long does a typical repair take?",
    a: "Most repairs are completed in just one visit, typically within 1-3 hours. Larger projects like popcorn ceiling removal or drywall installation may take longer, but we'll give you a clear timeline upfront.",
  },
  {
    q: "Do you match existing textures and paint?",
    a: "Absolutely! Texture matching is one of our specialties. We analyze your existing wall or ceiling texture and replicate it precisely so the repair is completely invisible.",
  },
  {
    q: "Are you licensed and insured?",
    a: "Yes, The Patch Boys is fully licensed and insured. We carry comprehensive liability insurance for your complete peace of mind.",
  },
  {
    q: "What areas in Bergen County do you serve?",
    a: "We serve all of Bergen County including Hackensack, Paramus, Teaneck, Fort Lee, Englewood, Ridgewood, Bergenfield, Fair Lawn, and all surrounding communities. If you're in Bergen County, we've got you covered!",
  },
  {
    q: "Can you fix water-damaged drywall?",
    a: "Yes, water damage repair is one of our core services. We remove the damaged section, treat for any moisture issues, install new drywall, and match the texture and paint for a seamless finish.",
  },
  {
    q: "Do I need to be home during the repair?",
    a: "We prefer that someone 18 or older be present during the repair. This allows us to discuss any findings and ensure you're completely satisfied with the work before we leave.",
  },
];
